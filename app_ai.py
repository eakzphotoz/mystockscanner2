...
with st.sidebar:
    ...
    if auto_refresh_enabled:
        ...
        # ตัวช่วยกระตุ้นการ Rerun อัตโนมัติด้วย JavaScript Meta tags หรือตัวควบคุมของ Streamlit
        st.empty()
...
# การทำงานของ Component ภายใต้ Fragment นี้จะไม่ทำให้หน้ากระตุกหรือกราฟรีโหลดใหม่เวลาโต้ตอบ!
@st.fragment
        ...
        if not df_s.empty:
            ...
            for idx, r in df_s.iterrows():
                ...
                with c1:
                    ...
                    """, unsafe_allow_html=True)
                with c2:
                    # เนื่องจากเราต้องอัปเดตหน้าหลักเพื่อเปลี่ยนสินทรัพย์ใน TradingView จึงต้องเรียก rerun ที่นี่
                    if st.button("วิเคราะห์ →", key=f"btn_sel_{t_ticker}_{portfolio_key}", use_container_width=True):
                        ...
                        st.session_state.ai_debate_result = None
                        st.session_state.chat_history = []
                        st.parent_rerun() # บังคับให้ Streamlit Rerun หน้าหลักทั้งหมดเพื่อเปลี่ยนข้อมูลกราฟด้านบน
...
st.markdown("<div style='text-align:center; color:#7b8494; font-size:0.75rem; margin-top:24px;'>"
            "ข้อมูลนี้ถูกประมวลผลด้วยโมเดลวิเคราะห์เชิงกลยุทธ์ Gemini 2.5-flash และ claude-3-5-sonnet เพื่อใช้เพื่อการศึกษาเทคโนโลยีการเงินเท่านั้น"
            "</div>", unsafe_allow_html=True)import streamlit as st
import streamlit.components.v1 as components
import yfinance as yf
import pandas as pd
import numpy as np
